...
char *FindDataLine(FILE *File_Ptr) {
    char *buf = malloc(STRLEN);  // Allocate heap memory
...
short ReadNumRuns(FILE* File_Ptr)
{
  ...
  char *tmp = FindDataLine(File_Ptr); /* skip file version. */
